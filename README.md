...
1) **Библиотеки и пространство имён**:
...
Если строка пуста или состоит из одного символа, возвращаем её как есть (переворачивать нечего).

**Рекурсивный вызов**:

**s.substr(1)** — создаёт подстроку, начиная со второго символа (индекс 1) до конца.

Пример: для "Hello" вернёт "ello".

**reverse_string(s.substr(1))** — рекурсивно переворачивает оставшуюся часть строки.

Пример: для "ello" вернёт "olle".

s[0] — берёт первый символ исходной строки.

Пример: для "Hello" это 'H'.

**Конкатенация**: результат рекурсивного вызова + первый символ.

Пример: "olle" + 'H' → "olleH"

**Функция main**

Создаёт строку text со значением "Hello World".

Вызывает **reverse_string(text)** и выводит результат через cout.

**endl** — переводит строку после вывода.
...
**Глубина рекурсии: O(n)**.
 ...
 Каждый рекурсивный вызов уменьшает строку на 1 символ.

 Для строки длины n будет n вызовов (от n до 1).

 **Стоимость одного вызова: O(n)**.

**s.substr(1)** создаёт новую строку, копируя n−1 символов. Это операция O(n).

**Конкатенация reverse_string(...) + s[0]** также требует копирования строки (в зависимости от реализации, но в худшем случае O(n)).

**Контрольный вопрос**: 4. Что такое мемоизация и как она помогает оптимизировать рекурсивные алгоритмы?

**Мемоизация** — это техника кэширования результатов выполнения функций для одних и тех же аргументов.

**Как помогает в рекурсии**:

Избегает повторных вычислений для одинаковых подзадач

Превращает экспоненциальную сложность в линейную

Сохраняет результаты в кэше (словаре) для мгновенного доступа
